...
const Chat = () => {
  ...
  return (
    <div id="chat" className="relative flex flex-col h-full overflow-hidden">
      {currentRoom && (
        <ChatHeader 
          user={user}
      ...
      <div className="flex flex-1 overflow-hidden min-h-0 h-full">
        ...
        {!isDesktop && videoOverlay === "chat" && ((callState!="idle" && isCaller) || (inCall && !isCaller)) && (
          <div
            className="md:hidden fixed bottom-[6.5%] left-0 right-0 h-[49%] 
        ...
        {videoOverlay === "members" && ((callState!="idle" && isCaller) || (inCall && !isCaller)) && (
          <div
            className="md:hidden fixed bottom-0 left-0 right-0 h-[51%] mb-10
